...
public readonly record struct RenderCommand(ObjectId ObjectId, Geometry2D Geometry, Transform2D WorldTransform, SceneStyle Style, BoundingBox2D Bounds);
...
public static class RenderPreparation
{
    ...
    public static RenderFrame Prepare(Scene2D scene, Camera2D camera) => new(scene.GetItems().Select(item => new RenderCommand(item.Id, item.Geometry, item.Transform, item.Style, item.Bounds)).ToArray(), camera.Target);
}
