...
public sealed class ACadSharpCadImporter : IDocumentImporter
{
    ...
    private static ImportResult ImportCore(ImportRequest request, IProgress<ImportProgress>? progress, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var entities = source.Entities.Select(entity => MapEntity(entity, diagnostics)).ToArray();
            var blocks = MapBlocks(source, diagnostics);
            ValidateBlockReferences(entities.Concat(blocks.SelectMany(block => block.Entities)), blocks, diagnostics);
            var metadata = new Dictionary<string, string> { ["Reader"] = "ACadSharp", ["ReaderVersion"] = typeof(CadDocument).Assembly.GetName().Version?.ToString() ?? "unknown", ["EntityCount"] = entities.Length.ToString(CultureInfo.InvariantCulture), ["BlockCount"] = blocks.Length.ToString(CultureInfo.InvariantCulture) };
    ...
    private static CadEntity MapEntity(Entity entity, List<Diagnostic> diagnostics)
    {
        var common = Common(entity);
        return entity switch
        {
            ...
            LwPolyline polyline => new CadPolylineEntity(common.Handle, polyline.Vertices.Select(vertex => Point(vertex.Location)).ToArray(), polyline.IsClosed, common.Layer, common.Color, common.Visible, common.LineType, common.LineWeight, common.Metadata),
            Polyline2D polyline => new CadPolylineEntity(common.Handle, Vertices(polyline).ToArray(), BoolProperty(polyline, "IsClosed"), common.Layer, common.Color, common.Visible, common.LineType, common.LineWeight, common.Metadata),
    ...
    private static CadBlockDefinition[] MapBlocks(global::ACadSharp.CadDocument source, List<Diagnostic> diagnostics)
    {
        ...
        foreach (var record in EnumerableProperty(source, "BlockRecords"))
        {
            ...
            var block = Property(record, "Block") ?? record; var basePoint = Point(Property(block, "BasePoint")); var entities = EnumerableProperty(record, "Entities").OfType<Entity>().Select(entity => MapEntity(entity, diagnostics)).ToArray();
    ...
    private static CommonEntity Common(Entity entity) => new(entity.Handle.ToString(CultureInfo.InvariantCulture), entity.Layer?.Name ?? "0", MapColor(entity.Color), !entity.IsInvisible, NameOf(entity.LineType), ParseLineWeight(entity.LineWeight), new Dictionary<string, string> { ["Normal"] = Property(entity, "Normal")?.ToString() ?? string.Empty, ["LineTypeScale"] = entity.LineTypeScale.ToString(CultureInfo.InvariantCulture) });
    ...
    private static IEnumerable<Point2D> Vertices(object source) => EnumerableProperty(source, "Vertices").Select(vertex => Point(Property(vertex, "Location") ?? Property(vertex, "Position")));
    ...
    private static bool BoolProperty(object source, string name) => Property(source, name) as bool? ?? false;
    private static int IntProperty(object? source, string name) => Convert.ToInt32(Property(source, name) ?? 0, CultureInfo.InvariantCulture);
